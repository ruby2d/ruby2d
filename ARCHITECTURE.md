...
**One clock.** All timing derives from the engine's monotonic clock: per-frame motion uses the shared frame delta (the `dt` passed to `update`, a.k.a. `Window#delta_time`) and absolute time uses `Window#elapsed`. No engine timing derives from `Time.now` or `Process.clock_gettime` (the sole wall-clock use is naming a screenshot file): they don't port to mruby (missing entirely, or overflowing the web build's 32-bit `mrb_int`), and one shared delta keeps all animation in lockstep, gets stalls clamped once centrally, and stays unit-testable (pass an explicit `dt`). New time-based behavior should consume `dt` (defaulting to `Window.delta_time`), never derive its own time. Behavior that advances only when drawn, such as a `Sprite` in the scene, reads `Window._clock`, the sum of those deltas, and charges itself its change since its previous update, so a tick that skips drawing in `:on_demand` mode delays the picture, not the animation.

**The canvas buffer is straight alpha.** A `Canvas` stores non-premultiplied RGBA, because the window blends the canvas texture as straight alpha. SDL's own blended blit writes premultiplied color, so over a transparent canvas pixel it would leave a color darkened by its own alpha, darkened again at display. Every blended write into the buffer therefore goes through `canvas_blend_over` in `ext/ruby2d/canvas.c`, including image and text stamps, which scale into a scratch surface first, while opaque writes and `clear` replace pixels outright; what a new drawing primitive must never use is an SDL blend blit.
