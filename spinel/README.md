...
The two examples it unblocks stop at gate 2, each on a new compiler shape in the example's own code, unreduced: `hill_driver.rb` does `accel ||= …` on a local of the enclosing `update` block from inside a nested `gamepads.each`, and the C reads an undeclared `lv_accel`; `platformer.rb` destructures with two throwaway parameters, `parts.each { |s, _, _| … }`, and the C defines `lv__` twice. Both are the next reductions.
