...
|---|---|---|
| 1. the preflight accepts it | `rake coverage` | 45 of 55 |
...
| 3. it draws the right pixels | `rake compare` | ten fixtures byte-identical to mruby; not measured per program |
...
`rake coverage` also ranks what to add next by how many programs each feature unblocks, greedily rather than by raw appearances: from 45 today, `Ellipse` first at +2, then `Audio`, `Sprite`, `SpriteSheet`, `Polygon`, `Tileset`, `BitmapText`, and the `Window` subclass pattern last. That ranking is about breadth only. It says nothing about cost — `Ellipse` and `Polygon` are geometry on proven patterns, `Sprite` is a clip on the `Image` handle that already takes one — and nothing about gates 3 and 4.
...
The `preflight` fixture moved to `Ellipse` and `Sprite`. Handles still leak — `Image` has no finalizer on this target any more than `Text` does.
