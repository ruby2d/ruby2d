...
|---|---|---|
| 1. the preflight accepts it | `rake coverage` | 47 of 55 |
...
| 3. it draws the right pixels | `rake compare` | eleven fixtures byte-identical to mruby; not measured per program |
...
`rake coverage` also ranks what to add next by how many programs each feature unblocks, greedily rather than by raw appearances: from 47 today, `Audio`, `Sprite`, `SpriteSheet`, `Polygon`, `Tileset`, `BitmapText` at one or two each, and the `Window` subclass pattern last. That ranking is about breadth only. It says nothing about cost — `Polygon` is geometry on proven patterns, `Sprite` is a clip on the `Image` handle that already takes one, `Audio` is a new subsystem — and nothing about gates 3 and 4.
...
What's left is coverage, not plumbing: the target draws `Square`, `Rectangle`, `Quad`, `Triangle`, `Circle`, `Ellipse`, `Line`, `Polyline`, `Text`, `Image`, `Canvas` and `Button`. The shapes draw **filled and stroked**, single-color or per-vertex, lines solid, dashed and gradient, text in any bundled or system font with styles, images in every format SDL_image decodes (SVG included) tinted, rotated and resized, and every canvas operation — each of which needed its own fixture before it could be trusted — see [Lessons](#lessons). An app using anything more stops before compiling with a message naming it — see [Preflight](#preflight).
...
| `undef_color` | `Canvas` and `Image` remove `Renderable`'s `color` accessors with `undef_method` (they take `fill` and `tint`); whole-program AOT has no method table to undefine from and refuses the call. Dropping the two lines loses nothing on this target. An AOT gap, not a bug — see [Deliberate feature gaps](#deliberate-feature-gaps-on-the-spinel-target). Was `canvas_undef_color` until `Image` joined on 2026-08-21 |
| `image_share_from` | `Image#initialize`'s `_share_from:` branch copies a `SpriteSheet`'s decoded texture with `instance_variable_get`, which is reflection. Neither class is on this target, so the branch becomes a raise. Revisit with `Sprite` — the clean fix is a protected reader in `lib/`, which is right on every engine |
...
**`Ellipse` draws, 45 → 47 programs accepted (85%), eleven fixtures byte-identical.** The smallest round so far: `R2D_DrawEllipse` and `R2D_StrokeEllipse` were Ruby-free already (`Circle` is the equal-radii case of both), so it was two `ffi_func`s, two adapters that float the radii and the tilt and integer the sectors, `ellipse` in the slice, and `ellipse_app.rb` — filled and stroked, tilted, orbiting a pivot, and stroke-only. `lib/` did not change; the fixture built on the first compile. The `preflight` fixture moved to `Polygon` and `Sprite`.

The two examples it unblocks stop at gate 2, each on a compiler shape in the example's own code.

**`hill_driver.rb` is [draft 57](issues/57-or-assign-to-a-captured-outer-local-inside-a-closure-reads-an-undeclared-identifier.md)**, five lines: `accel ||= …` on a local captured from outside a closure is lowered on the bare `lv_accel`, which the closure never declares. `&&=` and the `cache ||= compute` memo idiom hit it too; `+=`, a local first assigned inside the closure, and `x = x || …` all compile. What the first eleven probes missed is that the latch is a **top-level** local: defined inside the block, the same line compiles. Found by reading the C — `_cell_accel` was allocated at the top level, which pointed straight at lines 176–179 of the example.

**`platformer.rb` is parked, unreduced.** Its C shows two symptoms with one root: `parts` in `gems.each { |parts| parts.each { |s, _, _| … } }` is a poly `RbVal` there, so the inner block becomes a runtime `Proc` (`sp_proc_new_meta`) whose poly-argument prologue declares `lv__` once per `_`, and the update loop's `parts.each { … bob … }` becomes a closure whose `_cell_bob` is allocated in the wrong function. Every reduction kept `parts` typed, and then both paths inline and compile. Renaming the throwaways (`|s, _x, _y|`) clears the first symptom in the real program and not the second. The missing ingredient is whatever makes `gems` a `PolyArray` of `RbVal` — `gem_data.map do |x, y| build_gem(x, y, z: 3).map { |s| [s, s.x, s.y] } end`, captured by a lambda and the update block — and it did not survive being mirrored by hand.

## Image (2026-08-21)

**`Image` draws, 42 → 45 programs accepted (81%), ten `rake compare` fixtures byte-identical to mruby, and `--assets` works on this target.** `image.c` got the `Canvas` treatment: Ruby-free cores (`R2D_ImageLoadSurface`, `R2D_ImageDrawWith`, `R2D_ImageReload`, `R2D_ImageDestroy`) with the bindings reading ivars and delegating, and a handle API on top — `R2D_ImageNew(path, w, h)` decodes (the size steers the SVG rasterizer), `R2D_ImageDraw` takes the geometry, pivot, tint, a flip in `SDL_FlipMode`'s bits and a scale mode by name, `R2D_ImageClip` parks clip and trim state on the handle for `Sprite` to set before each draw, and `R2D_ImageResize` re-rasterizes. `Canvas#draw_image` works through `R2D_CanvasDrawImageNamed`. The adapter copies the decoded size back with `_spinel_size` and the clip reset with `_spinel_loaded`, so `image_create` and `image_resize` write exactly what C writes on the other engines.

With a class that reads files, `--assets` and `# ruby2d:assets` directives stop being refused: `build_spinel` validates and bundles the directories with the same `bundle_asset_dirs` the mruby build uses. `image_app.rb` declares the gem's `assets/test_media/images` that way, and `compare.rb` copies a fixture's directive directories into its scratch dir so the same source builds on both engines. The four `test/` programs that use `Ruby2D.test_images` pass preflight now; they build the way they do for mruby, through `rake test:spinel <name>`, which rewrites those paths to a bundled `media/` directory as `test:native` does — `ruby2d build --spinel test/image.rb` on its own has no `Ruby2D.test_images`, on either compiled engine. `test/image.rb` and `test/canvas.rb` build and run; `test/window.rb` calls `public_send`, which is the test's own AOT gap.

Two things in `lib/` changed, both small:

| Draft | Shape | Change |
|---|---|---|
| [53](issues/53-multiple-assignment-from-ivars-emits-invalid-c-when-a-keyword-can-reassign-one.md) | the same massign-from-ivars shape as `Canvas#render` | `Image#render` saves and restores with plain assignments |
| [56](issues/56-a-block-over-a-memoized-sort-by-result-is-mistyped-when-another-class-has-a-method-named-like-the-singleton-method.md) | `Font.path` stopped compiling the moment a class with a `path` method existed: the block over the memoized `sort_by` list is typed `const char *` and dispatched as poly at once. Three conditions, and the name collision is one of them | `Font` memoizes the filesystem scan (`find_os_font_files`) instead of the filtered list, and `Font.all` / `Font.path` share a `name_of` helper |

Draft 56 took most of the morning, and the lesson is the control. An app that instantiates nothing is **not** a baseline: Spinel leaves the ivars of never-constructed classes untyped, so `Polyline#width`, `Color#to_a` and `String#error` all fail to compile in a program that draws nothing, and those failures read exactly like the real one. Bisect against a fixture that builds, never against an empty window. The other thing that looked like a regression and was not: `ruby2d build --spinel --debug` drops the `-w`, so it shows the four `-Werror=incompatible-pointer-types` diagnostics (drafts 46 and 47) that every normal build silences.

The `preflight` fixture moved to `Ellipse` and `Sprite` (and on to `Polygon` the same day). Handles still leak — `Image` has no finalizer on this target any more than `Text` does.
...
Two AOT gaps got transforms rather than `lib/` changes — `canvas_undef_color` (since renamed `undef_color`) and `button_visual_hooks`, both in [Workarounds to re-check](#workarounds-to-re-check) — because the code they neutralize is right on the other engines and there is nothing to reduce.
