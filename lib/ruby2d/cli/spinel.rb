...
#
# Passing the block positionally keeps the capture. Both callbacks keep their
# block form as well, so only the generated top-level shims change — everything
# else that calls them is untouched. Delete once #3783 is fixed.
def spinel_positional_callbacks(src)
  src = spinel_sub(src,
                   "    def update(&proc)\n" \
                   ...
                   "      proc = positional || blk\n" \
                   "      raise Error, '`update` requires a block' unless proc\n",
                   'Window#update positional block')
  src = spinel_sub(src,
                   "    def render(z: :foreground, &proc)\n" \
                   "      raise Error, '`render` requires a block' unless proc\n",
                   "    def render(positional = nil, z: :foreground, &blk)\n" \
                   "      proc = positional || blk\n" \
                   "      raise Error, '`render` requires a block' unless proc\n",
                   'Window#render positional block')
  # The shims are generated by spinel_dsl_shims, so this half runs on the
  # assembled program rather than on `lib/` — see spinel_assemble.
  src = spinel_sub(src,
                   "def update(&proc)\n  Ruby2D::DSL.window.update(&proc)\nend\n",
                   "def update(&proc)\n  Ruby2D::DSL.window.update(proc)\nend\n",
                   'top-level update shim')
  spinel_sub(src,
             "def render(z: :foreground, &proc)\n  Ruby2D::DSL.window.render(z: z, &proc)\nend\n",
             "def render(z: :foreground, &proc)\n  Ruby2D::DSL.window.render(proc, z: z)\nend\n",
             'top-level render shim')
end
...
def spinel_assemble(app_source, lib_dir:, ffi: true)
  ...
  # Runs here rather than in spinel_compat: half its sites are in the shims
  # above, which don't exist until they're generated.
  spinel_positional_callbacks(src)
end
...
def spinel_unsupported_uses(source)
  ...
  source.lines.each_with_index do |line, i|
    ...
    found << [n, 'on', 'input events — event dispatch needs a runtime `send`'] if line.match?(/(\A\s*|\.)on(\(|\s+[:\w])/)
