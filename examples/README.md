...
- Two keys that pull one value opposite ways (left and right arrows) get a latch each (`steer_left`, `steer_right`), and the signed value is derived from the pair each frame; one signed latch loses the key still held when the other is released.
- A mouse interaction (a drag, a stroke, held spawning) belongs to the button that started it: record that button, ignore another pressed meanwhile, and end the interaction only on its release, or filter both handlers to one button (`on mouse_down: :left`, `on mouse_up: :left`). An unfiltered `:mouse_up` ends a left-button drag when the right button is tapped.
