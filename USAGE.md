...
`contains?` (and the object events built on it: `:click`, `:hover`, `:drag`, …) respects rotation: the clickable region matches the rotated shape you see, not its unrotated footprint. It tests against the **rendered fill**: for `Triangle`, `Quad`, and `Polygon` that's the filled area. For `Line` and `Polyline` it matches the drawn stroke, within half the stroke width along each segment, mitered at the corners, and butt-capped at the open ends (so it doesn't extend past them); a dashed `Line` is hit only on its dashes, not in the gaps. A `Circle` or `Ellipse` with fewer than 30 `sectors` is drawn as a visibly faceted polygon and hit-tests against that polygon; from 30 up it tests the ideal curve, which the drawn rim tracks closely. A `Triangle` is always convex and hit-tests exactly against what's drawn, as do convex and concave `Quad`s and `Polygon`s. A `Tileset` is hit on its placed tiles, each following its type's `rotate:`, and nowhere else: with no tiles placed it contains nothing. Only self-intersecting `Quad`s and `Polygon`s are unreliable (see [Polygon](#polygon)), so `contains?` may not match the drawn pixels there. Points exactly on an edge follow the rasterized boundary (the left and top edges count as inside, the right and bottom edges as outside), for `Rectangle`, `Image`, `Text`, and every other box as well, so two objects that share an edge never both claim a point on it.
...
A `Polygon` fills any simple outline, convex or concave, whichever vertex it starts from. Self-intersecting outlines (a bow tie, a star drawn in one stroke) aren't supported: the fill may cover only part of the shape or a region outside it, and `Canvas#fill_polygon` shares the same triangulation. Draw each lobe as its own `Polygon`.
...
Placing a tile at a coordinate that's already occupied replaces the existing one. The `flip:` option for `define` accepts `:horizontal`, `:vertical`, or `:both`. Mouse events and `contains?` test against the placed tiles (see [Per-Object Events](#per-object-events)).
...
Events are dispatched to the topmost (highest z-order) interactive object at the mouse position; among equal z, the object drawn on top (the most recently added) wins, and a Button ranks by its visual. A press brings hover up to date before `:mouse_down`, so an object that appeared under a resting cursor gets its `:hover` on the first click, and the object hovered before gets `:hover_out`.
