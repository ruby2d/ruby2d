...
def test_image(file_name)
  "#{Ruby2D.test_images}/#{file_name}"
end
...
shared_examples 'image format' do |format_ext|
  context "loading #{format_ext}" do
    let(:path) { test_image "image.#{format_ext}" }
    it 'succeeds' do
...
#
# ==Image-loading Tests==
# Here we have a set of tests that are common to Image and Sprite.
# When writing a spec for those classes (and others that have the same
# common properties), pull it in as follows:
#
# ```
...
shared_examples 'image-loading tests' do
  ...
  context 'using pathname' do
    let(:path) { Pathname.new(test_image('image.png')) }
    it 'converts path to string' do
      expect(subject.path.is_a?(String)).to be true
    end

    it 'succeeds' do
      expect(subject.path).to end_with test_image('image.png')
    end
  end
end
...
shared_examples 'image-like tests' do |klass|
  it "creates #{klass.name} with valid tint" do
    expect(subject.tint).to be_a Ruby2D::Color
  end

  it "creates #{klass.name} with white tint by default" do
    expect(subject.tint.to_a).to eq [1, 1, 1, 1]
  end
  ...
  context 'with options' do
    ...
    it 'succeeds' do
      ...
      expect(subject.tint.r).to eq(2 / 3.0)
      expect(subject.tint.opacity).to eq(0.5)
    end
  end
end
